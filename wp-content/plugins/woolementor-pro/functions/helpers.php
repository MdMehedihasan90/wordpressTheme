...
function woolementor_pro_license_activated() {
	...
	if( !class_exists( '\codexpert\Product\License' ) ) return false;

	$plugin = get_plugin_data( WOOLEMENTOR_PRO );
	$plugin['file'] = WOOLEMENTOR_PRO;
	$plugin['server'] = apply_filters( 'woolementor-pro_server', 'https://my.codexpert.io' );
	
	$license = new \codexpert\Product\License( $plugin, 'admin.php?page=woolementor#woolementor_license' );
