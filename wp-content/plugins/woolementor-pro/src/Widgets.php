...
class Widgets extends Base {
	...
	public function add_control( $element, $section_id, $args ) {
		if ( 'section' === $element->get_name() && 'section_structure' == $section_id ) {
			...
			$element->add_control(
				...
				[
					'type'			=> Controls_Manager::SWITCHER,
					'label'			=> __( 'Contains Thankout?', 'woolementor' ),
					'description'	=> __( 'Does this section contain Woolementor Thankout page', 'woolementor' ),
					'separator'		=> 'before',
				]
			);
	...
	public function stop_render_checkout( $should_render, $object )	{
		...
		if ( isset( $settings['contains_checkout'] ) && $settings['contains_checkout'] == 'yes'  && is_order_received_page() ) {
			return false;
		}
